...
import { BIOLOGY_CHAPTER_03 } from './questions.js';
import {
  createIcons,
  AlertTriangle,
  Home,
...
function renderComparisonTable(tableData, columnsToKeep) {
  ...
  const filteredHeaders = activeIndices.map(idx => headers[idx]);

  let html = `<table class="responsive-comparison-table">`;
  html += `<thead><tr>`;
  ...
  rows.forEach(row => {
    html += `<tr>`;
    activeIndices.forEach((headerIdx, displayIdx) => {
      const label = filteredHeaders[displayIdx];
      const key = keys[headerIdx];
      const cellValue = row[key] || "";
      const isAspect = (displayIdx === 0);
      const cellClass = isAspect ? 'class="font-extrabold text-primary-purple"' : '';
      html += `<td data-label="${escapeHtml(String(label))}" ${cellClass}>${escapeHtml(String(cellValue))}</td>`;
    });
    html += `</tr>`;
  });
...
// --- GENERIC EQUATION RENDERER (reads structured term data, no hardcoded IDs) ---
function renderEquation(eqData, isWrong) {
  const cls = isWrong ? ' bg-red-50 border-red-200 text-red-700' : '';
  const opCls = isWrong ? ' text-red-500' : '';
  const arrowCls = isWrong ? ' text-red-500' : '';
  const renderTerms = (terms) => terms.map((t, i) => {
    const sep = i < terms.length - 1 ? `<span class="equation-operator${opCls}">+</span>` : '';
    return `<span class="equation-term${cls}"><bdi>${escapeHtml(t)}</bdi></span>${sep}`;
  }).join('');
  return `
    <div class="scientific-equation" dir="rtl">
      <div class="reactants flex flex-wrap items-center gap-1.5">${renderTerms(eqData.reactants)}</div>
      <div class="equation-arrow${arrowCls}">←</div>
      <div class="products flex flex-wrap items-center gap-1.5">${renderTerms(eqData.products)}</div>
    </div>
  `;
}

// --- GENERIC MODEL-ANSWER RENDERER ---
// Reads structured rendering metadata from questions.js (answerRenderMeta / comparisonTables / equations).
// Contains no question-ID-specific educational text; all wording lives in questions.js.
function formatModelAnswer(qId, rawAnswer) {
  const data = window.BIOLOGY_CHAPTER_03;
  const meta = data && data.answerRenderMeta ? data.answerRenderMeta[qId] : null;
  if (!meta) return `<span class="whitespace-pre-wrap">${escapeHtml(rawAnswer || "")}</span>`;

  const tables = data.comparisonTables || {};
  const equations = data.equations || {};

  if (meta.type === "comparison") {
    const table = tables[meta.table];
    if (!table) return `<span class="whitespace-pre-wrap">${escapeHtml(rawAnswer || "")}</span>`;
    return `
      <div class="my-2">
        <p class="mb-3 font-extrabold text-main-text text-sm">${escapeHtml(meta.intro)}</p>
        ${renderComparisonTable(table, meta.columns)}
      </div>
    `;
  }

  if (meta.type === "equation") {
    const eq = equations[meta.equation];
    if (!eq) return `<span class="whitespace-pre-wrap">${escapeHtml(rawAnswer || "")}</span>`;
    return `
      <div class="my-2 flex flex-col gap-2">
        <p class="font-bold text-main-text text-sm">${escapeHtml(meta.intro)}</p>
        ${renderEquation(eq)}
      </div>
    `;
  }

  if (meta.type === "correction-branches") {
    const branchesHtml = (meta.branches || []).map(b => `
      <p class="font-bold text-main-text text-sm mb-2">
        <span>${escapeHtml(b.label)}</span> ${escapeHtml(b.text)}
      </p>
    `).join('');

    let equationBlock = '';
    if (meta.equationPair) {
      const wrongEq = equations[meta.equationPair.wrong];
      const correctEq = equations[meta.equationPair.correct];
      equationBlock = `
        <div class="border-t border-gray-100 pt-3 mt-2">
          <p class="text-xs font-bold text-red-600 mb-1">المعادلة المغلوطة (الافتراضية في السؤال):</p>
          ${wrongEq ? renderEquation(wrongEq, true) : ''}
        </div>
        <div class="mt-2">
          <p class="text-xs font-bold text-green-600 mb-1">المعادلة الكيميائية الصحيحة (الجواب النموذجي):</p>
          ${correctEq ? renderEquation(correctEq, false) : ''}
        </div>
      `;
    }
  ...
  return `<span class="whitespace-pre-wrap">${escapeHtml(rawAnswer || "")}</span>`;
}

const STORAGE_KEY = "school_biology_muscular_system_v3";
const DB_NAME = "biology_drawings_db_ch03";
const STORE_NAME = "student_drawings_ch03";
...
let activeIdx = 0;
let currentFilter = "all"; // "all" | "unanswered" | "unrated" | "rating_8_10" | "rating_5_7" | "rating_1_4"
...
function getFilteredQuestions() {
  const list = activeSection === "source" 
    ? window.BIOLOGY_CHAPTER_03.sourceQuestions 
    : window.BIOLOGY_CHAPTER_03.enrichmentQuestions;
    
  return list.filter(q => {
    const isAttempted = isQuestionAttempted(q);
    const isRated = appState.ratings[q.id] !== undefined;
    const isHigh = appState.mastery[q.id] === "high";
    const isMid = appState.mastery[q.id] === "mid";
    const isLow = appState.mastery[q.id] === "low";

    if (currentFilter === "unanswered") return !isAttempted;
    if (currentFilter === "unrated") return !isRated;
    if (currentFilter === "rating_8_10") return isHigh;
    if (currentFilter === "rating_5_7") return isMid;
    if (currentFilter === "rating_1_4") return isLow;
    return true;
  });
}
...
function renderApp() {
  ...
  // Re-generate bundled local Lucide icons (no CDN/network dependency).
  createIcons({ icons: LOCAL_LUCIDE_ICONS });
}

// --- HOME SCREEN ---
function renderHomeScreen() {
  const sourceLen = window.BIOLOGY_CHAPTER_03.sourceQuestions.length;
  const enrichmentLen = window.BIOLOGY_CHAPTER_03.enrichmentQuestions.length;
  
  const sourceAttempted = window.BIOLOGY_CHAPTER_03.sourceQuestions.filter(isQuestionAttempted).length;
  const enrichmentAttempted = window.BIOLOGY_CHAPTER_03.enrichmentQuestions.filter(isQuestionAttempted).length;
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        ...
        <div class="bg-white border border-borders p-6 rounded-3xl shadow-sm flex flex-col justify-between gap-5 relative group hover:border-primary-purple/40 transition-all">
          <div class="flex flex-col gap-3">
            ...
            <p class="text-xs font-bold leading-relaxed text-muted-text">
              تضم هذه المجموعة ${sourceLen} سؤالاً مأخوذة بشكل مباشر من الكتاب المدرسي لضمان التمكن التام من المنهج.
            </p>
          </div>
        ...
        <div class="bg-white border border-borders p-6 rounded-3xl shadow-sm flex flex-col justify-between gap-5 relative group hover:border-primary-purple/40 transition-all">
          <div class="flex flex-col gap-3">
            ...
            <p class="text-xs font-bold leading-relaxed text-muted-text">
              تضم هذه المجموعة ${enrichmentLen} سؤالاً إثرائياً تم صياغتها بعناية لاختبار المفاهيم وتوسيع الفهم العلمي للموضوع.
            </p>
          </div>
...
function renderQuestionScreen() {
  ...
  let paginationHtml = `
    <div class="flex flex-col gap-2 mt-4">
      ...
      <div class="flex gap-1.5 pb-2 no-scrollbar w-full flex-wrap justify-center">
  `;
  ...
  if (question.questionType === "multi-part") {
    ...
    (question.subItems || []).forEach((sub, sIdx) => {
      ...
      if (sub.questionType === "written") {
        ...
        subInputArea = `
          <div class="flex flex-col gap-1">
            <textarea data-sub-id="${sub.id}" rows="3" placeholder="اكتب إجابتك هنا..." class="sub-written-input w-full p-3 rounded-xl border border-borders bg-white focus:border-primary-purple focus:outline-none text-sm leading-relaxed" ${isShown ? "disabled" : ""}>${escapeHtml(val)}</textarea>
          </div>
        `;
      ...
      if (isShown) {
        subModelAnswerHtml = `
          <div class="bg-green-50/70 border border-green-200 rounded-xl p-3.5 mt-3 flex flex-col gap-1 text-right">
            ...
            <p class="text-xs font-bold text-main-text leading-relaxed whitespace-pre-wrap">${escapeHtml(sub.modelAnswer || "")}</p>
          </div>
        `;
      }
      ...
      subItemsHtml += `
        <div class="border border-borders/80 bg-page-bg/10 rounded-2xl p-4 md:p-5 flex flex-col gap-3">
          <h3 class="text-sm font-black text-main-text leading-relaxed">${escapeHtml(sub.question || "")}</h3>
          ${subInputArea}
          ${subModelAnswerHtml}
        </div>
      `;
    ...
    inputAreaHtml = `
      <div class="flex flex-col gap-2">
        ...
        <textarea id="written-input" rows="4" placeholder="اكتب محاولتك هنا..." class="w-full p-3 rounded-xl border border-borders bg-white focus:border-primary-purple focus:outline-none text-sm leading-relaxed" ${isShown ? "disabled" : ""}>${escapeHtml(val)}</textarea>
      </div>
    `;
    ...
    inputAreaHtml = `
      <div class="flex flex-col gap-4">
        ...
        <div id="correction-wrapper" class="flex flex-col gap-2 ${isFalseSelected ? "" : "hidden"}">
          ...
          <input type="text" id="correction-input" value="${escapeHtml(tfObj.correction || "")}" placeholder="اكتب تصحيح العبارة المناسب هنا..." class="w-full p-2.5 rounded-lg border border-borders bg-white focus:border-primary-purple focus:outline-none text-sm" ${isShown ? "disabled" : ""}>
        </div>
      </div>
    `;
    ...
    inputAreaHtml = `<div id="drawing-area">${buildDrawingAreaHtml(question)}</div>`;
  }

  // --- Reusable drawing-area builder (upload control + preview + status) ---
  // Regenerated in isolation after upload/delete (see attachDrawingAreaListeners) — never via full renderApp().
  function buildDrawingAreaHtml(q) {
    const hasImage = !!drawingImages[q.id];
    const base64 = drawingImages[q.id] || "";
    return `
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center gap-3 flex-wrap sm:flex-nowrap">
            <label id="upload-label" class="bg-white border border-borders hover:border-primary-purple hover:bg-soft-lavender/40 transition-all font-bold text-xs py-2.5 px-4 rounded-xl shadow-sm cursor-pointer flex items-center gap-1.5">
              ...
              <input type="file" id="drawing-file-input" class="hidden" accept="image/png,image/jpeg,image/webp">
            </label>
  ...
  // --- Reusable answer-area builder (reveal button + model answer + rating grid) ---
  // Used both for the initial paint and for partial re-render on reveal-click,
  // so revealing an answer never triggers a full renderApp()/page remount.
  function buildAnswerAreaHtml(q, shown) {
    let revealHtml = "";
    if (q.questionType !== "drawing" && !shown) {
      revealHtml = `
    ...
    let modelHtml = "";
    let ratingHtml = "";
    if (shown && q.questionType !== "drawing") {
      const displayedAnswer = q.studentDisplayAnswer || q.modelAnswer || "";
      const formattedAnswer = formatModelAnswer(q.id, displayedAnswer);
      modelHtml = `
        <div class="bg-green-50/50 border border-green-200 rounded-2xl p-5 mt-6 flex flex-col gap-3 font-serif">
          ...
          <div class="model-answer-text text-sm font-medium leading-relaxed text-main-text">${formattedAnswer}</div>
        </div>
      `;

      const currentRating = appState.ratings[q.id] || null;
      let gridButtons = "";
      for (let r = 1; r <= 10; r++) {
    ...
    return `${revealHtml}${modelHtml}${ratingHtml}`;
  }

  const answerAreaHtml = buildAnswerAreaHtml(question, isShown);
  ...
  setTimeout(() => {
    ...
    // MCQ clicks — toggle selection classes in place, no remount
    ...
    mcqBtns.forEach(btn => {
      btn.addEventListener("click", safeAction((e) => {
        ...
        mcqBtns.forEach((b, i) => {
          if (i === idx) {
            b.classList.add("bg-primary-purple", "border-primary-purple", "text-white", "shadow-sm");
            b.classList.remove("bg-white", "border-borders", "text-muted-text", "hover:border-primary-purple", "hover:bg-page-bg/10");
          } else {
            b.classList.remove("bg-primary-purple", "border-primary-purple", "text-white", "shadow-sm");
            b.classList.add("bg-white", "border-borders", "text-muted-text", "hover:border-primary-purple", "hover:bg-page-bg/10");
          }
        });
        updateRevealButtonState();
      }));
    ...
    // True/False toggles — update classes and correction-wrapper visibility in place, no remount
    ...
    const correctionInputEl = document.getElementById("correction-input");

    function applyTfSelectionToDom(selected) {
      if (tfTrue) {
        if (selected === true) {
          tfTrue.classList.add("bg-green-600", "border-green-600", "text-white", "shadow-sm");
          tfTrue.classList.remove("bg-white", "border-borders", "text-muted-text");
        } else {
          tfTrue.classList.remove("bg-green-600", "border-green-600", "text-white", "shadow-sm");
          tfTrue.classList.add("bg-white", "border-borders", "text-muted-text");
        }
      }
      if (tfFalse) {
        if (selected === false) {
          tfFalse.classList.add("bg-red-600", "border-red-600", "text-white", "shadow-sm");
          tfFalse.classList.remove("bg-white", "border-borders", "text-muted-text");
        } else {
          tfFalse.classList.remove("bg-red-600", "border-red-600", "text-white", "shadow-sm");
          tfFalse.classList.add("bg-white", "border-borders", "text-muted-text");
        }
      }
      if (correctionWrapper) {
        if (selected === false) {
          correctionWrapper.classList.remove("hidden");
        } else {
          correctionWrapper.classList.add("hidden");
        }
      }
    }
    ...
    if (tfTrue && tfFalse) {
      tfTrue.addEventListener("click", safeAction(() => {
        ...
        saveAppState();
        if (correctionInputEl) correctionInputEl.value = "";
        applyTfSelectionToDom(true);
        updateRevealButtonState();
      }));
      tfFalse.addEventListener("click", safeAction(() => {
        const prev = appState.tfAnswers[question.id] || {};
        appState.tfAnswers[question.id] = { selected: false, correction: prev.correction || "" };
        saveAppState();
        applyTfSelectionToDom(false);
        updateRevealButtonState();
      }));
    }

    // Correction text input
    if (correctionInputEl) {
      correctionInputEl.addEventListener("input", (e) => {
    ...
    // Drawing upload and deletion — regenerate only #drawing-area, no full remount
    function attachDrawingAreaListeners() {
        ...
        fileInput.addEventListener("change", (e) => {
          ...
          if (file) {
            ...
            reader.onload = async () => {
              ...
              if (area) {
                area.innerHTML = buildDrawingAreaHtml(question);
                attachDrawingAreaListeners();
              }
              updateRevealButtonState();
            };
    ...
    // Sub-MCQ clicks — toggle selection classes within the same branch group only, no remount
    ...
    subMcqBtns.forEach(btn => {
      btn.addEventListener("click", safeAction((e) => {
        ...
        saveAppState();
        const groupBtns = document.querySelectorAll(`.sub-mcq-option-btn[data-sub-id="${subId}"]`);
        groupBtns.forEach(b => {
          const bIdx = parseInt(b.getAttribute("data-opt-idx"));
          if (bIdx === idx) {
            b.classList.add("bg-primary-purple", "border-primary-purple", "text-white", "shadow-sm");
            b.classList.remove("bg-white", "border-borders", "text-muted-text", "hover:border-primary-purple", "hover:bg-page-bg/10");
          } else {
    ...
    // Reveal + rating — regenerate only #answer-area, no full remount
    function attachAnswerAreaListeners() {
      const revealBtn = document.getElementById("reveal-btn");
      if (revealBtn) {
        revealBtn.addEventListener("click", safeAction(() => {
          if (appState.shownAnswers[question.id]) return; // already revealed; button shouldn't exist, but guard anyway
          appState.shownAnswers[question.id] = true;
          saveAppState();
          const area = document.getElementById("answer-area");
          if (area) {
            area.innerHTML = buildAnswerAreaHtml(question, true);
            attachAnswerAreaListeners();
          }
        }));
      }
      ...
      rateBtns.forEach(btn => {
        btn.addEventListener("click", safeAction((e) => {
          const val = parseInt(e.currentTarget.getAttribute("data-rate"));
          ...
          rateBtns.forEach(b => {
            const bv = parseInt(b.getAttribute("data-rate"));
            if (bv === val) {
              b.classList.add("bg-primary-purple", "border-primary-purple", "text-white", "shadow-sm", "scale-105");
              b.classList.remove("bg-white", "border-borders", "text-muted-text");
            } else {
              b.classList.remove("bg-primary-purple", "border-primary-purple", "text-white", "shadow-sm", "scale-105");
              b.classList.add("bg-white", "border-borders", "text-muted-text");
            }
          });
        }));
      });
    ...
    // Prev / Next click — genuine navigation to a different question: full renderApp() is appropriate here.
  ...
  return `
    <div class="flex flex-col gap-6 max-w-3xl mx-auto w-full">
      ...
      <div class="bg-white border border-borders rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-5 relative mt-2">
        ...
        <!-- Answer area: reveal button, model answer, rating grid. -->
        <!-- Regenerated in isolation on reveal-click (see attachAnswerAreaListeners) — never via full renderApp(). -->
        <div id="answer-area">
          ${answerAreaHtml}
        </div>
      </div>
  ...
  return `
    <div class="flex flex-col gap-6 max-w-3xl mx-auto w-full">
      ...
      <div class="bg-white border border-borders rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-5 mt-2">
        ...
        <!-- Rating distribution grid (numeric ranges only) -->
